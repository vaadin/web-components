...
describe('DatePicker', () => {
  it('should apply "value" the last', async () => {
    ...
    const { container } = await render(<DateTimePicker label="Foo" value={value} step={1} />);
  ...
  it('should not override value set on DOM element on rerender', async () => {
    const { container, rerender } = await render(<DateTimePicker label="Foo" />);
    ...
    await rerender(<DateTimePicker label="Foo" />);
    expect(element).to.have.value('2020-01-01T15:00');
  });
});
