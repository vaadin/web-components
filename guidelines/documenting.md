...
CEM's output, the per-package `custom-elements.json`, drives four things:
...
- The per-package `vue.d.ts`, which augments Vue's `GlobalComponents` so
  `vue-tsc` and Volar type-check `<vaadin-*>` tags in templates. Property
  and event types are indexed access into the `.d.ts` (`TextField['value']`,
  `TextFieldEventMap['value-changed']`), so an event missing from the
  `*EventMap` falls back to a plain `CustomEvent` handler type.

CEM is configured in `custom-elements-manifest.config.js` at the repo
root; `scripts/split-cem.js` splits the merged manifest per package,
`scripts/buildWebtypes.js` turns it into the two web-types files, and
`scripts/generate-vue-types.js` writes the Vue declarations, checked by
`yarn lint:types:vue` against the fixtures in `test/typings/vue/`.
