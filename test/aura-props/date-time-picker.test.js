...
// TODO: --vaadin-input-field-container-gap is not documented on the date-time-picker styling page
// but is used in field-base-styles.js:19 to set the --_gap property on the host.

export const props = [
  // === Component-Specific ===
  {
    name: '--vaadin-date-time-picker-gap',
    value: '30px',
    compute(element) {
      const inputFields = element.shadowRoot.querySelector('[part="input-fields"]');
      return getComputedStyle(inputFields).getPropertyValue('gap').trim();
    },
  },
...
describe('date-time-picker', () => {
  let element;
  beforeEach(async () => {
    element = fixtureSync('<vaadin-date-time-picker></vaadin-date-time-picker>');
