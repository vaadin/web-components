...
describe('overlay in master-detail-layout', () => {
  ...
  it('should fire backdrop-click on backdrop click when no field overlay is open', async () => {
    await clickBackdrop();

    expect(spy).to.be.calledOnce;
  });

  ['vaadin-combo-box', 'vaadin-date-picker'].forEach((tagName) => {
    describe(tagName, () => {
      let field;

      beforeEach(async () => {
        field = layout.querySelector(tagName);
        field.open();
        await oneEvent(field.$.overlay, 'vaadin-overlay-open');
      });

      it('should not fire backdrop-click when the click closes the field overlay', async () => {
        await clickBackdrop();
      ...
      it('should fire backdrop-click on the click after the field overlay is closed', async () => {
        await clickBackdrop();
        await clickBackdrop();
