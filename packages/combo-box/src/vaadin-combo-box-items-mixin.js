...
export const ComboBoxItemsMixin = (superClass) =>
  class ComboBoxItemsMixinClass extends ComboBoxBaseMixin(superClass) {
    ...
    _onOpened() {
      ...
      if (this.filter && !this._focusModel.hasFocusedItem) {
        this._focusModel.focusItemAt(this.__getItemIndexByFilter(this._dropdownItems));
      }
    }
    ...
    _setDropdownItems(newItems) {
      ...
      this.__restoreItemFocus(focusedItem);
    }
    ...
    __restoreItemFocus(item) {
      ...
      if (
        item instanceof ComboBoxPlaceholder &&
        items[this._focusModel.focusedItemIndex] instanceof ComboBoxPlaceholder
      ) {
        return;
      }

      const index = this.__getItemIndexByValue(items, this._getItemValue(item));
      this._focusModel.focusItemAt(index > -1 ? index : this.__getItemIndexByFilter(items));
    }
