...
export const ComboBoxMixin = (subclass) =>
  class ComboBoxMixinClass extends OverlayClassMixin(
    ...
    willUpdate(props) {
      ...
      if (props.has('items')) {
        const oldItems = props.get('items');
      ...
      if (props.has('opened') || props.has('filteredItems') || props.has('loading')) {
        // Close the overlay if there are no items to display.
        // See https://github.com/vaadin/vaadin-combo-box/pull/964
        this._overlayOpened = !!(this.opened && (this.loading || (this.filteredItems && this.filteredItems.length)));
      }

      if (props.has('filteredItems')) {
        this._filteredItemsChanged(this.filteredItems, props.get('filteredItems'));
      }

      if (props.has('selectedItem') || props.has('itemValuePath') || props.has('itemLabelPath')) {
    ...
    _initScroller(host) {
      ...
      // Create the scroller once to ensure it's always the same
    ...
    _updateScroller(scroller, items, opened, loading, selectedItem, itemIdPath, focusedIndex, renderer, theme) {
      if (scroller) {
        ...
        scroller.itemIdPath = itemIdPath;
    ...
    /** @private */
    _getItemLabel(item) {
      let label = item && this.itemLabelPath ? this._get(this.itemLabelPath, item) : undefined;
    ...
    /** @private */
    _getItemValue(item) {
      let value = item && this.itemValuePath ? this._get(this.itemValuePath, item) : undefined;
      if (value === undefined) {
    ...
    _onInput(event) {
      ...
      // TODO: move `setProperties` to `PolylitMixin`
