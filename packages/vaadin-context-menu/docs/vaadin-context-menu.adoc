...
By default, [vaadinelement]#vaadin-context-menu# itself is used as the target to listen to [propertyname]#vaadin-contextmenu# events.
In practice, any child of the target element will respond to [propertyname]#vaadin-contextmenu# events for opening the menu.
This event can be customized as explained in the following chapters.

[NOTE]
[propertyname]#vaadin-contextmenu# is a custom event which acts like the native [propertyname]#contextmenu# event, but works also in iOS
where the native [propertyname]#contextmenu# event is not supported.
...
By default, the overlay opens when the [propertyname]#listenOn# element receives a [propertyname]#vaadin-contextmenu# event.
...
Overlay can be also opened programmatically by calling the [methodname]#open(event)#. It takes an
event as the argument, which is used to position the overlay correctly.
