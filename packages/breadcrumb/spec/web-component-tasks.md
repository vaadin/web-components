...
Create the `@vaadin/breadcrumb` package with empty shells of all three elements (`<vaadin-breadcrumb>`, `<vaadin-breadcrumb-item>`, `<vaadin-breadcrumb-overlay>`) using the standard mixin chains called out in `WEB_COMPONENT_GUIDELINES.md`. Each element registers, has a `static get is()`, an empty `render()`, an empty base styles file, and matching `.d.ts` files. The container is wired up with the experimental feature flag (`breadcrumbComponent`) per `@vaadin/component-base/src/define.js` so it requires `window.Vaadin.featureFlags.breadcrumbComponent = true`. A smoke test confirms the elements register and render without errors.

**Files:**
- `packages/breadcrumb/package.json` (create)
- `packages/breadcrumb/LICENSE` (create — Apache 2.0)
- `packages/breadcrumb/README.md` (create)
- `packages/breadcrumb/vaadin-breadcrumb.js` (create — root export)
- `packages/breadcrumb/vaadin-breadcrumb.d.ts` (create — root types)
...
- `packages/breadcrumb/src/vaadin-breadcrumb-mixin.js` (create — empty mixin shell)
...
- `packages/breadcrumb/src/styles/vaadin-breadcrumb-base-styles.js` (create — empty `css\`\``)
...
- `packages/breadcrumb/src/styles/vaadin-breadcrumb-overlay-base-styles.js` (create — empty)
- `packages/breadcrumb/src/styles/vaadin-breadcrumb-overlay-base-styles.d.ts` (create)
- `packages/breadcrumb/test/breadcrumb.test.ts` (create — smoke test)
- `packages/breadcrumb/test/typings/breadcrumb.types.ts` (create — empty type test)
...
**Tests:**
- [ ] Without setting the feature flag, importing the package does not register `vaadin-breadcrumb` (the registry returns `undefined`)
- [ ] After setting `window.Vaadin.featureFlags.breadcrumbComponent = true`, all three tag names (`vaadin-breadcrumb`, `vaadin-breadcrumb-item`, `vaadin-breadcrumb-overlay`) are present in `customElements`
- [ ] Each element's `is` static getter equals its tag name
- [ ] Creating an empty `<vaadin-breadcrumb>` and inserting it into the DOM does not throw
...
- [ ] With `path` unset, the shadow DOM contains a `<span part="link">` and no `<a>`
- [ ] Setting `path` after the element is rendered switches the `<span>` to an `<a>` with the new href
- [ ] Setting `target="_blank"` while `path` is set adds `target="_blank"` to the `<a>`
- [ ] The default slot's text content is wrapped in a `<span part="label">`
- [ ] Adding the `current` attribute on the host adds `aria-current="page"` to the inner link/span
- [ ] Removing the `current` attribute removes `aria-current` from the inner link/span
...
- [ ] Setting `--vaadin-breadcrumb-separator: url(...)` on the host changes the `mask-image` of `::after` to the new URL
...
**Spec sections:** `<vaadin-breadcrumb>` (Shadow DOM, Slots, Parts, Internal behavior — Slot observation and root assignment)
**Requirements:** 1, 10
**Depends on:** 1, 3
...
**Spec sections:** Key Design Decisions §3, `<vaadin-breadcrumb>` (Internal behavior — Slot observation and root assignment, last paragraph re: current state)
**Requirements:** 2, 3, 11
**Depends on:** 6
...
- [ ] The inner link/span of the current item carries `aria-current="page"` (regression check for the integration with Task 3)
...
## Task 8: Container `items` programmatic property

**Spec sections:** Key Design Decisions §4, `<vaadin-breadcrumb>` (Properties — `items`, Internal behavior — `items` property), Types (`BreadcrumbItemData`)
**Requirements:** 9
**Depends on:** 6, 7, 3
...
- `packages/breadcrumb/src/vaadin-breadcrumb-mixin.js` (modify)
- `packages/breadcrumb/src/vaadin-breadcrumb-mixin.d.ts` (modify)
- `packages/breadcrumb/src/vaadin-breadcrumb.d.ts` (modify — re-export `BreadcrumbItemData`)
- `packages/breadcrumb/test/breadcrumb.test.ts` (modify)
- `packages/breadcrumb/test/typings/breadcrumb.types.ts` (modify — assert `BreadcrumbItemData` shape)

**Tests:**
- [ ] Setting `items` on an empty breadcrumb renders one `<vaadin-breadcrumb-item>` light-DOM child per array entry
- [ ] Each generated item's text content equals the corresponding entry's `text`
- [ ] Entries with `path` produce items with the correct `path` attribute; entries without `path` produce items without the attribute
- [ ] After `items` is set, the first generated item carries `slot="root"` and the last (when it has no `path`) carries `current` (delegated to Tasks 6 and 7)
- [ ] Setting `items` replaces any pre-existing slotted children
- [ ] Setting `items = null` removes the generated items and restores any original author-supplied children
- [ ] Reassigning `items` to a new array with different length / entries updates the rendered children to match
...
## Task 9: Container overflow detection via `ResizeMixin`

**Spec sections:** Key Design Decisions §6, `<vaadin-breadcrumb>` (Properties / State attribute / Parts — `has-overflow`, `overflow`, `overflow-button`, Internal behavior — Overflow detection)
**Requirements:** 6
**Depends on:** 6, 7
...
**Files:**
- `packages/breadcrumb/src/vaadin-breadcrumb-mixin.js` (modify)
- `packages/breadcrumb/src/styles/vaadin-breadcrumb-base-styles.js` (modify — `:host(:not([has-overflow])) [part='overflow'] { display: none }`, `[data-overflow-hidden] { display: none }`)
- `packages/breadcrumb/test/breadcrumb.test.ts` (modify)
...
## Task 10: Container overflow separator pseudo-element

**Spec sections:** `<vaadin-breadcrumb>` (Internal behavior — Overflow separator)
**Requirements:** 4, 12
**Depends on:** 2, 9

Render `[part="overflow"]::after` using the same `mask-image` + `currentColor` + `--vaadin-breadcrumb-separator` pattern used on `<vaadin-breadcrumb-item>::after` (Task 5), with the same `transform: scaleX(-1)` flip in RTL. Because the overflow element is hidden when `has-overflow` is unset, the separator is implicitly hidden then too — no additional rule is needed.
...
**Tests:**
- [ ] When `has-overflow` is set, `getComputedStyle(overflowEl, '::after')` reports a non-`none` `mask-image` matching the configured `--vaadin-breadcrumb-separator`
...
- [ ] Overriding `--vaadin-breadcrumb-separator` on the host updates the `mask-image` of the overflow `::after`
...
## Task 11: `<vaadin-breadcrumb-overlay>` element and `BreadcrumbOverlayMixin`

**Spec sections:** `<vaadin-breadcrumb-overlay>` (full section, including Shadow DOM and Internal behavior)
**Requirements:** 7
**Depends on:** 1

Build the `<vaadin-breadcrumb-overlay>` element on top of `OverlayMixin` (from `@vaadin/overlay/src/vaadin-overlay-mixin.js`) plus a new `BreadcrumbOverlayMixin` that holds any breadcrumb-specific overlay tweaks (mirrors how `<vaadin-combo-box-overlay>` pairs `OverlayMixin` with `ComboBoxOverlayMixin`). Render the standard `[part="overlay"]` / `[part="content"]` shadow structure with a default slot inside content. The element exposes no public properties of its own — it inherits `opened`, `owner`, `renderer`, and `positionTarget` from `OverlayMixin`.

**Files:**
- `packages/breadcrumb/src/vaadin-breadcrumb-overlay.js` (modify)
- `packages/breadcrumb/src/vaadin-breadcrumb-overlay.d.ts` (modify)
- `packages/breadcrumb/src/vaadin-breadcrumb-overlay-mixin.js` (create — `BreadcrumbOverlayMixin`)
- `packages/breadcrumb/src/vaadin-breadcrumb-overlay-mixin.d.ts` (create)
- `packages/breadcrumb/src/styles/vaadin-breadcrumb-overlay-base-styles.js` (modify — minimal structural rules; theme polish later)
- `packages/breadcrumb/test/breadcrumb-overlay.test.ts` (create)

**Tests:**
- [ ] `customElements.get('vaadin-breadcrumb-overlay')` returns a class
...
- [ ] `BreadcrumbOverlayMixin` is in the element's class hierarchy (e.g. an `instanceof`-style check via a marker exposed by the mixin, or by reading the prototype chain)
...
## Task 12: Container overlay integration and `i18n.moreItems`

**Spec sections:** Key Design Decisions §6, `<vaadin-breadcrumb>` (Shadow DOM — `<vaadin-breadcrumb-overlay>` block, Properties — `i18n`, Internal behavior — Overlay management)
**Requirements:** 6, 7
**Depends on:** 9, 11

Render `<vaadin-breadcrumb-overlay>` directly in the container's shadow DOM with `.owner`, `.opened`, `.renderer`, and `.positionTarget` bound to the host's state and the overflow button (also pass `exportparts="overlay, content"` so themes can target both via `vaadin-breadcrumb-overlay::part(...)`; project a `<slot name="overlay">` inside it for consistency with other overlay hosts). Clicking the overflow button toggles `_overlayOpened`. The `renderer` writes one navigable link into `[part="content"]` per currently hidden item, in original DOM order, using each item's `path` as href and its text content as the link label. Apply `I18nMixin` with the `i18n` object containing `moreItems` (default `''`); bind it to the overflow button's `aria-label`. Keep `aria-haspopup="true"` and update `aria-expanded` on the overflow button to reflect open/closed.
...
**Spec sections:** Key Design Decisions §7, §8; `<vaadin-breadcrumb>` (host role, `role="list"` wrapper, overflow button ARIA); `<vaadin-breadcrumb-item>` (host role, `aria-current`)
**Requirements:** 10, 11
**Depends on:** 7, 12
...
**Files:**
- `packages/breadcrumb/test/breadcrumb-a11y.test.ts` (create)

**Tests:**
- [ ] `<vaadin-breadcrumb>` host has `role="navigation"` and an application-supplied `aria-label` on the host is preserved (the component does not set or overwrite it)
...
## Task 14: Keyboard interaction verification

**Spec sections:** `<vaadin-breadcrumb>` (Internal behavior — Overlay management); `<vaadin-breadcrumb-item>` (Link rendering)
**Requirements:** 1, 7
**Depends on:** 12
...
**Files:**
- `packages/breadcrumb/test/breadcrumb-keyboard.test.ts` (create)
...
- [ ] The current item's inner `<span>` (no `path`) is not in the Tab cycle
...
## Task 15: RTL verification

**Spec sections:** `<vaadin-breadcrumb-item>` (Internal behavior — RTL separator flip); `<vaadin-breadcrumb>` (Internal behavior — Overflow separator)
**Requirements:** 12
**Depends on:** 5, 10, 12
...
**Files:**
- `packages/breadcrumb/test/breadcrumb-rtl.test.ts` (create)
- `packages/breadcrumb/src/styles/vaadin-breadcrumb-base-styles.js` (modify if needed to switch any physical properties to logical equivalents)
...
## Task 16: Base styles completion and base visual tests

**Spec sections:** `<vaadin-breadcrumb>` (Parts, State attributes, CSS Custom Property), `<vaadin-breadcrumb-item>` (Parts, State attributes), `<vaadin-breadcrumb-overlay>` (Parts)
...
- [ ] Visual baseline: applying a custom `--vaadin-breadcrumb-separator` (e.g. a slash icon) changes the separator across all items and the overflow
...
**Spec sections:** `<vaadin-breadcrumb>` (Parts, CSS Custom Property), `<vaadin-breadcrumb-item>` (Parts, State attributes), `<vaadin-breadcrumb-overlay>` (Parts)
...
## Task 19: Dev page, snapshots, type tests, final validation

**Spec sections:** All
**Requirements:** All
**Depends on:** 8, 12, 17, 18

Add the dev page in `dev/breadcrumb.html` with all major variants (declarative, with icons, with `items` property, in a resizable container so the overflow can be exercised manually, current-page omitted, custom separator, RTL). Add DOM snapshot tests for the host shadow DOM in default and key states (basic, with overflow, with current item, with `items` set). Flesh out the TypeScript type tests so every public property and the `BreadcrumbItemData` interface are exercised. Run the full validation suite for the package.
...
- `packages/breadcrumb/test/dom/breadcrumb-overlay.test.js` (create)
- `packages/breadcrumb/test/typings/breadcrumb.types.ts` (modify — assert `items: BreadcrumbItemData[] | null | undefined`, `i18n.moreItems`, `path`/`target` on item, event types)

**Tests:**
- [ ] DOM snapshot: `<vaadin-breadcrumb>` shadow DOM in the default state
- [ ] DOM snapshot: `<vaadin-breadcrumb>` shadow DOM with `has-overflow` set
- [ ] DOM snapshot: `<vaadin-breadcrumb-item>` shadow DOM with `path` set
- [ ] DOM snapshot: `<vaadin-breadcrumb-item>` shadow DOM without `path` and with `current` set
- [ ] DOM snapshot: `<vaadin-breadcrumb-item>` shadow DOM with prefix slot content
- [ ] DOM snapshot: `<vaadin-breadcrumb-overlay>` shadow DOM
- [ ] Type test: `BreadcrumbItemData` requires `text: string` and accepts an optional `path?: string`
- [ ] Type test: `breadcrumb.items` accepts `BreadcrumbItemData[] | null | undefined`
- [ ] Dev page loads in both Lumo and Aura without console errors and exposes the overflow / RTL / `items`-property variants
