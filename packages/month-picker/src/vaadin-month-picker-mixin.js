...
export const MonthPickerMixin = (superClass) =>
  class MonthPickerMixinClass extends I18nMixin(InputMixin(superClass)) {
    ...
    __commitChanges(selectedValue, yearMonth) {
      ...
      // The user committed an unparsable / out-of-range value. Don't
      // silently mutate `value` — leave the previously parsed value as is
      // and notify the application via `unparsable-change`. Keep
      // `_lastCommittedValue` aligned so the next valid commit fires
