...
describe('virtualizer - item height - initial render', () => {
  ...
  describe('small item height', () => {
    ...
    it('should have the expected amount of physical elements', () => {
      expect(elementsContainer.childElementCount).to.equal(19);
    });
    ...
    it('should call updateElement for each item once', () => {
      expect(updateElement).to.have.callCount(19);
    });
  });
...
describe('virtualizer - item height - resize with late idle callback', () => {
  ...
  it('should keep number of physical elements reasonable when item heights decrease', async () => {
    ...
    expect(scrollTarget.querySelectorAll('.item').length).to.be.at.most(itemCount + 4);
  });
});
