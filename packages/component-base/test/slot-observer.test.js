...
describe('SlotObserver', () => {
  ...
  describe('target mode', () => {
    ...
    it('should run callback again after reconnect and flush', async () => {
      spy = sinon.spy();
      observer = new SlotObserver(target, spy);
      await Promise.resolve();
      spy.resetHistory();
    ...
    it('should run callback with forceInitial: true when target has no slot descendants', async () => {
      const empty = document.createElement('div');
      empty.attachShadow({ mode: 'open' });
      document.body.appendChild(empty);

      spy = sinon.spy();
      observer = new SlotObserver(empty.shadowRoot, spy, true);
      await Promise.resolve();

      expect(spy.calledOnce).to.be.true;
      const { currentNodes } = spy.firstCall.args[0];
      expect(currentNodes).to.be.empty;

      empty.remove();
    });

    it('should pick up a slot added to the target after construction', async () => {
      const empty = document.createElement('div');
      empty.attachShadow({ mode: 'open' });
      document.body.appendChild(empty);
      empty.innerHTML = '<div slot="late">child</div>';

      spy = sinon.spy();
      observer = new SlotObserver(empty.shadowRoot, spy);
      await Promise.resolve();
      spy.resetHistory();

      const newSlot = document.createElement('slot');
      newSlot.setAttribute('name', 'late');
      empty.shadowRoot.appendChild(newSlot);
      ...
      expect(spy.calledOnce).to.be.true;
      const { addedNodes } = spy.firstCall.args[0];
      expect(addedNodes).to.deep.equal([empty.firstElementChild]);

      empty.remove();
    });

    it('should expose deduped currentNodes after each change', async () => {
      spy = sinon.spy();
      observer = new SlotObserver(target, spy);
      await Promise.resolve();
      spy.resetHistory();
      ...
      const { currentNodes } = spy.firstCall.args[0];
      const unique = new Set(currentNodes);
      expect(unique.size).to.equal(currentNodes.length);
      expect(currentNodes).to.include(div);
    });
  });
});
