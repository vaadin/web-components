...
The breadcrumbs display a horizontal sequence of items representing the path from the root of a hierarchy to the user's current location. Each ancestor item is a navigable link that takes the user directly to that level when activated.
...
The application can choose whether the current-page item is included in the trail. When omitted, the breadcrumbs show only the ancestor links leading up to (but not including) the current page.

*Example: A documentation portal shows the page title as a heading below the breadcrumbs. The breadcrumbs display "Home > Developer Guide > API Reference" without repeating "Authentication" because the heading already identifies the current page.*
...
The breadcrumbs can be updated at runtime to reflect a hierarchy that changes based on application state, user navigation, or data-driven paths. The component renders whatever trail the application provides, without assuming a fixed sitemap.

*Example: A product catalog's breadcrumbs show "Electronics > Laptops > Gaming" when browsing by category. When the user switches to browsing by brand, the application updates the trail to "Brands > Asus > Laptops", and the breadcrumbs immediately reflect the new path.*

---

## 10. Navigation landmark

The breadcrumbs identify themselves as a navigation landmark so that assistive technology users can locate them among other landmarks on the page.

*Example: A screen reader user pressing a landmark shortcut finds the breadcrumbs navigation region without having to read through every link on the page.*
...
*Example: A screen reader user tabbing through the breadcrumbs hears each ancestor announced as a link. The last item is announced as "OAuth2, current page", making clear that this is their current location rather than a navigable link.*
...
*Example: In an Arabic-language application, the breadcrumbs read from right to left and the chevron separators point left instead of right, so the visual flow matches the reading direction.*

---

## 13. Flow: Default trail derived from the router hierarchy

In the Flow wrapper, instantiating Breadcrumbs without configuring its items causes it to populate itself automatically from the application's router. The default strategy walks up the URL path of the current route and matches each prefix to a registered Flow route, producing one breadcrumb item per matched ancestor. The text shown for each item is the matched route's view title (the value Flow uses to set the page title for that route — typically the route's `@PageTitle`, or the title supplied dynamically by the view).

*Example: A user navigates to `/customers/acme/orders`. The orders view is annotated `@PageTitle("Orders")`, the customer detail view dynamically sets its title to "Acme Corp", the customers list view is `@PageTitle("Customers")`, and the root view is `@PageTitle("Home")`. A view that simply does `add(new Breadcrumbs())` shows "Home › Customers › Acme Corp › Orders" — one item per matched route, each labeled with that route's view title.*

---

## 14. Flow: Opting out of automatic trail population

The Flow wrapper lets the application disable automatic population so the breadcrumbs show only the items the application provides explicitly. Opting out is a per-instance choice and does not affect other Breadcrumbs instances in the application.

*Example: A product catalog view builds its trail from category data rather than the URL structure. It opts the breadcrumbs out of automatic population and supplies the items itself, so router-derived ancestors are not mixed in.*
...
*Example: A route registered at `/orders/edit/123` declares its sitemap parent as the orders list view. The breadcrumbs show "Home › Orders › Edit Order" instead of the URL-derived "Home › Orders › Edit › Edit Order".*

---

## 16. Flow: Routes can dynamically supply their breadcrumbs contribution

A Flow `@Route` class can implement an interface that lets it dynamically provide what should appear in the breadcrumbs for that view. The contribution is evaluated at navigation time, so it can depend on the data the view is currently showing, and it can replace the view's default item, add additional ancestors, or both.

*Example: A `CustomerView` showing customer "Acme Corp" implements the interface and returns "Acme Corp" as its own label plus an extra ancestor "Enterprise" (the customer's segment). The breadcrumbs show "Home › Customers › Enterprise › Acme Corp" instead of a generic "Home › Customers › Customer".*
...
**Q: When the breadcrumbs are too long to fit the available width, how should the component handle overflow?**
...
**Q: On small viewports (mobile), should the breadcrumbs simplify their display?**
...
**Q: Should the navigation landmark have a default label (e.g., "Breadcrumbs")?**
...
A Flow application already declares its sitemap as `@Route` classes, so requiring every view to manually build the breadcrumbs duplicates information the framework already has. Defaulting to router-derived items (req 13) lets the trivial case work with zero configuration. Apps that need full control can opt out per instance (req 14).
