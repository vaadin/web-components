...
- **Decisions:**
  ...
  - `role="listitem"` added to `<vaadin-breadcrumbs-item>` host in `firstUpdated()` (only if absent) — Spec Decision 8 and Task 5's user-value statement mandate it. Surfaced by review; the previous tasks (1–4) hadn't added it. Two new ARIA-roles tests in `breadcrumbs-item.test.js`.
  - Current marking driven by a `MutationObserver` on the host with `{ childList: true, subtree: true, attributeFilter: ['path'] }`. `subtree: true` is **required** to observe `path` mutations on direct children — without it, the observer only sees the host's own attributes. The fix agent verified this when the initial drop-subtree:true attempt broke the `setAttribute('path', ...)` test.
  - `__updateCurrent()` iterates `this.children` filtered by `localName === 'vaadin-breadcrumbs-item'`, calls `item._setCurrent(isCurrent)` on every item (true on the last iff its `path == null`, false otherwise to clear stale state). The defensive `typeof item._setCurrent === 'function'` check guards against pre-upgrade items.
  - Observer disconnected in `disconnectedCallback`, re-observed in `connectedCallback` to handle host detach/reattach. `firstUpdated` fires once and sets up the initial observer; the `if (this.__childObserver)` guard makes the first `connectedCallback` (before `firstUpdated`) a no-op.
  ...
  - Visual test for the default trail in `test/visual/base/`. Forced-colors visual test deliberately omitted: the runner has no `emulateMedia` infrastructure, and a stub passing-test would be misleading. The decision is documented in the visual test file's header comment.
- **Surprises:**
  ...
  - The fix agent's first attempt to drop `subtree: true` from the MutationObserver setup broke two tests. `subtree: true` is the right setting for observing `path` attribute changes on direct children — they are descendants of the observation root.
- **Spec adjustments:** —
