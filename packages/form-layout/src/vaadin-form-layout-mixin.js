...
export const FormLayoutMixin = (superClass) =>
  class extends SlotStylesMixin(ResizeMixin(ControllerMixin(superClass))) {
    ...
    connectedCallback() {
      ...
      requestAnimationFrame(() => this.__updateLayout());
    }
    ...
    __autoResponsiveControllerPropsChanged(
      ...
      this.__autoResponsiveController.updateProps({
        columnWidth,
        maxColumns,
        autoRows,
        labelsAside,
      ...
      if (autoResponsive) {
        this.addController(this.__autoResponsiveController);
      } else {
        this.removeController(this.__autoResponsiveController);
      }
    }
  };
