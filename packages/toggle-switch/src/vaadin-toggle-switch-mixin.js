 ...
 */
export const ToggleSwitchMixin = (superclass) =>
  class ToggleSwitchMixinClass extends SlotStylesMixin(
    ...
    constructor() {
      ...
      // Set the string "on" as the default value for the toggle switch following the HTML specification:
    ...
    ready() {
      ...
      this.addController(
        new InputController(this, (input) => {
          ...
          // Set role="switch" on the hidden input for proper ARIA semantics
          input.setAttribute('role', 'switch');
        }),
      );
      this.addController(new LabelledInputController(this.inputElement, this._labelController));

      // Registered in `ready()` so the observer does not fire for the
      // initial `checked` value — matches the behavior the validation
      // tests rely on.
    ...
    updated(changed) {
      ...
      if (changed.has('readonly') || changed.has('inputElement')) {
        this.__readonlyChanged(this.readonly, this.inputElement);
      }
    }
