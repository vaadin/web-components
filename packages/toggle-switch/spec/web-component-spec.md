...
5. **Hide the checkmark marker drawn by `checkable()` on `[part='switch']::after`.** The shared style paints a checkmark mask intended for a checkbox tick. The toggle switch shows its on-state via thumb position and track color, not a glyph, so the local stylesheet clears `background` and `mask` on `[part='switch']::after` and adds a separate `[part='thumb']` element inside the track. The pseudo-element itself is preserved so its em-space content keeps providing the flex container's first baseline — `checkable()`'s `:host { align-items: baseline }` grid rule depends on that baseline to align the label's text baseline with the centre of the track.
...
**Q: Why is Task 3's behavioral coverage so small?**

`guidelines/12-testing.md` states "don't test mixin / controller internals in components that consume them — the mixin's own tests already cover that." `CheckboxMixin` (with its inherited mixin chain) is exhaustively tested in `packages/checkbox/test/`; the toggle switch reuses that mixin without modification, so duplicating the same assertions here would only create maintenance overhead. The toggle-switch-specific surface — `role="switch"` on the inner input, the track / thumb / required-indicator / tooltip-slot template — is locked in by the snapshot tests at `packages/toggle-switch/test/dom/`. The shared tooltip integration test at `test/integration/component-tooltip.test.js` exercises the tooltip wiring across every Vaadin field, including `<vaadin-toggle-switch>`. Beyond those two coverage points, no toggle-switch-specific unit tests are warranted.

**Q: Why are there no unit tests for the `--vaadin-toggle-switch-*` CSS custom property overrides?**

Computed-style assertions that override a custom property and read back the resulting computed value test the CSS engine, not the component. Visual regression tests already lock in the rendered appearance for every state the property affects, and consumers verify their token overrides by themselves overriding the property in their own styles. Per `guidelines/12-testing.md` ("Prefer visual tests over unit tests for CSS"), the base visual suite at `test/visual/base/toggle-switch.test.ts` is the canonical coverage for the styling layer.
