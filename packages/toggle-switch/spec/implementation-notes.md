...
- **Decisions:**
  ...
  - CSS custom-property coverage: `test/styles.test.ts` is a parameterised computed-style suite. A single `TESTS` table lists `{ prop, override, target, read, checked? }` per property and a `forEach` generates one `it` block per row. The test agent picked `host` / `[part='switch']` / `[part='thumb']` / `[part='label']` per the spec's intent for each property; all 13 pass.
- **Surprises:**
  - The test agent's `read: (s: CSSStyleDeclaration) => string` property type tripped `@typescript-eslint/method-signature-style`. The implementation agent ran `eslint --fix` on the test file, which converted it to a method shorthand. Pure mechanical change, behavior-preserving. The review agent confirmed no need to respawn the test agent.
