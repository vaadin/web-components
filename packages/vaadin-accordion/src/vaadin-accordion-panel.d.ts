/**
 * @license
 * Copyright (c) 2019 - 2022 Vaadin Ltd.
 * This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
 */
import type { AccordionPanel } from '@vaadin/accordion/src/vaadin-accordion-panel.js';

/**
 * @deprecated Import `AccordionPanel` from `@vaadin/accordion/vaadin-accordion-panel` instead.
 */
export type AccordionPanelElement = AccordionPanel;

/**
 * @deprecated Import `AccordionPanel` from `@vaadin/accordion/vaadin-accordion-panel` instead.
 */
export const AccordionPanelElement: typeof AccordionPanel;

export * from '@vaadin/accordion/src/vaadin-accordion-panel.js';
