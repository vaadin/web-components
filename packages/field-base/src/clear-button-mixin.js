...
export const ClearButtonMixin = (superclass) =>
  class ClearButtonMixinClass extends InputMixin(KeyboardMixin(superclass)) {
    ...
    _onClearButtonMouseDown(event) {
      if (!this._shouldBlurOnClearMousedown(event)) {
        event.preventDefault();
      }
    ...
    /**
     * Whether to blur the field on clear button mousedown.
     * By default, if the field currently has focus, it is
     * preserved by calling `preventDefault()` on mousedown
     * event in order to avoid blur and change events.
     *
     * @protected
     * @return {boolean}
     */
    _shouldBlurOnClearMousedown() {
