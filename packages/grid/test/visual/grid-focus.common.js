    ...
    describe(dir, () => {
      ...
      describe('first header row', () => {
        beforeEach(async () => {
          ...
          await sendKeys({ press: isRTL ? 'ArrowRight' : 'ArrowLeft' });
        });

        it('default', async () => {
          await visualDiff(element, `${dir}-first-header-row`);
        });

        it('first cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await visualDiff(element, `${dir}-first-header-row-first-cell`);
        });

        it('last cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await sendKeys({ press: 'End' });
          await visualDiff(element, `${dir}-first-header-row-last-cell`);
        });
      });
      ...
      describe('last header row', () => {
        ...
        it('default', async () => {
          await visualDiff(element, `${dir}-last-header-row`);
        });

        it('first cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await visualDiff(element, `${dir}-last-header-row-first-cell`);
        });

        it('last cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await sendKeys({ press: 'End' });
          await visualDiff(element, `${dir}-last-header-row-last-cell`);
        });
      });
      ...
      describe('first body row', () => {
        ...
        it('default', async () => {
          await visualDiff(element, `${dir}-first-body-row`);
        });

        it('first cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await visualDiff(element, `${dir}-first-body-row-first-cell`);
        });

        it('last cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await sendKeys({ press: 'End' });
          await visualDiff(element, `${dir}-first-body-row-last-cell`);
        });
      });
      ...
      describe('last body row', () => {
        ...
        it('default', async () => {
          await visualDiff(element, `${dir}-last-body-row`);
        });

        it('first cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await visualDiff(element, `${dir}-last-body-row-first-cell`);
        });

        it('last cell', async () => {
          await sendKeys({ press: isRTL ? 'ArrowLeft' : 'ArrowRight' });
          await sendKeys({ press: 'End' });
          await visualDiff(element, `${dir}-last-body-row-last-cell`);
        });
      });
    });
