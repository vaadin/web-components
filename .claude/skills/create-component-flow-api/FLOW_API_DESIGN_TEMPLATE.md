...
<!--
Flow (Java) developer-facing API derived from requirements.md (universal + flow) and the web-component web-component-api.md. Shows the most idiomatic, minimal Java API a Flow developer would use.
...
The Flow API ALWAYS wraps the web component. Every attribute/property/slot/event/CSS custom property in web-component-api.md must be reachable from Java — see the Web API coverage check at the end.
...
<!--
Verify every web-component API surface from web-component-api.md is reachable from Flow. For each item, state the Java API that exposes it, or explicitly mark it as "internal / not reachable from Flow" with a rationale.
-->
