...
1. Read `packages/{component-name}/spec/developer-api.md`. This is the primary input — the ideal developer-facing API from step 3. If the file does not exist, stop and tell the user to run `create-component-api-design` first.
...
6. Reconcile the ideal API with source code findings. For each feature in `developer-api.md`: can it be implemented as-is using existing patterns? Does naming need adjustment for consistency with existing components? Can an existing mixin be reused or extended? Would a shared module need modification? Document every deviation from `developer-api.md` in the Key Design Decisions section with rationale.
...
- **Key Design Decisions** — One entry for each significant choice. Include deviations from `developer-api.md` with rationale. Reference developer-api.md sections by number/name when discussing the API feature that motivated the decision. Include decisions about mixin selection, slot design, and event naming.
...
- Do not modify `developer-api.md`, `requirements.md`, or `problem-statement.md`.
