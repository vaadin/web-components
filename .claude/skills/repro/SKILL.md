...
5. **Resolve every component the bug names to its real source before scaffolding — never assume markup or API from memory.** Element tags, attributes, and theme idioms drift across versions, and a remembered convention is often wrong for the current checkout. A wrong guess here aims the entire reproduction at the wrong artifact (e.g. testing a `<span theme="badge">` Lumo idiom when the repo actually ships a `<vaadin-badge>` element with `number`/`theme` attributes and its own injected styles — a different element, shadow DOM, and styling lifecycle). For each component or feature the bug mentions, confirm what it *is* in this repo:
   - **Web:** check for `packages/<name>/` and its element definition (`grep -rln "customElements.define\|static get is" packages/<name>/src`), the entry import (`@vaadin/<name>`), and any `dev/<name>.html` example. Build the scaffold from the **real element and its current API**, copying markup from the existing `dev/<name>.html` rather than writing it from training memory. If a `packages/<name>` exists, prefer the component over a same-named legacy `theme="…"` styling idiom unless the reporter clearly means the latter.
   - **Flow:** find the Java component class and package under `vaadin-<name>-flow-parent/`, and prefer the API as used in existing integration-test views.
6. **Confirm the intended behavior**, then form a hypothesis:
...
7. **Look for a duplicate.** Search **open and closed** issues in the same repo for the same component plus the distinctive symptom — an exception class + line, a unique error string, or the specific trigger. Omit `--state` so it covers all states — `gh search issues` rejects `--state all` (it only accepts `open`/`closed`):
   ```bash
...
Build the smallest view that exercises your hypothesis, using the **real component API confirmed in Phase 1.5** — the actual element/import and an existing `dev/<name>.html` example, not markup recalled from training. Prefer the reporter's code example as the starting point; include only what the hypothesis needs (the real minimization happens in Phase 3, once it reproduces). If there is no example, construct one from the description and the trigger Y.
