...
A bug usually lives in one repo. If the issue is filed against `flow-components` but the root cause is in the web component, reproduce on the web side first (it is faster) — but follow whichever flavor the reporter's code example uses.
