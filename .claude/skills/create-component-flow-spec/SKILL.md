...
1. Read `packages/{component-name}/spec/flow-developer-api.md`. This is the primary input — the ideal Flow API from `create-component-flow-api-design`. If the file does not exist, stop and tell the user to run `create-component-flow-api-design` first.

2. Read `packages/{component-name}/spec/developer-api.md`. This is the web component API the Flow wrapper exposes from Java. If it does not exist, stop and tell the user to run `create-component-api-design` first.

3. Read `packages/{component-name}/spec/spec.md` if it exists. The web-component spec lists the internal web API — shadow DOM parts, slots, events, synchronised properties, CSS custom properties — that the Flow wrapper may need to hook into (e.g. via `@Synchronize` or `Element.setPropertyJson`). If it does not exist, note this as a dependency: some Flow spec decisions may need revisiting once the web component spec is written.
...
8. Study existing flow-components source. Extract class names mentioned in `flow-developer-api.md` rationale sections (e.g. "follows `HasSideNavItems` pattern") and use those as starting points. Read:
...
9. Reconcile the ideal Flow API with source findings. For each feature in `flow-developer-api.md`:
   ...
   Document every deviation from `flow-developer-api.md` in the Key Design Decisions section with rationale.
...
11. Verify coverage. Cross-reference the spec against `requirements.md` (universal + flow only) — every requirement must be addressed in the spec. Cross-reference against `flow-developer-api.md` — every API feature is either present in the spec or has a documented deviation in Key Design Decisions. Cross-reference against the Web API coverage check in `flow-developer-api.md` — every web-component API surface that was mapped to a Flow API must now have a concrete class/method/field entry in the spec.
...
- **Completeness over minimalism.** Every universal/flow requirement must be covered. Every web-component API surface from `developer-api.md` must be reachable from Flow (per the coverage check in `flow-developer-api.md`).
...
- **Key Design Decisions** — one entry per significant choice. Deviations from `flow-developer-api.md` with rationale. Mixin-interface selections. Connector decision with rationale.
...
- Do not modify `flow-developer-api.md`, `developer-api.md`, `requirements.md`, `problem-statement.md`, `spec.md`, or any existing files outside the new spec file.
- Do not read source code outside `flow-components/` (or `../flow-components/`). The web component spec is available at `packages/{name}/spec/spec.md` — read that, not the web component source under `packages/*/src/`.
