...
10. Read the relevant chapters of the `guidelines/` folder — especially `02-design.md` (UX, behavior, and design principles) and `11-a11y.md` (accessible names, customizable labels, focus order). DO NOT write numbered requirements that restate those universal rules (accessible names, customisable labels, focus order matching visual order, right-to-left support, readable/tappable targets on small viewports). They are enforced globally and would drift if duplicated per component.
...
13. No requirement restates a universal behavioural rule from `guidelines/02-design.md` or `guidelines/11-a11y.md` (accessible names, customisable labels, focus order, RTL, tappable-on-small-screens). Requirements that touch those concerns must add a component-specific default, extension, or interaction pattern — otherwise remove them.
