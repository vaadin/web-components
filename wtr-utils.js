...
const isLockfileChanged = () => {
  const log = execSync('git diff --name-only origin/25.1 HEAD').toString(); // NOSONAR
...
const getChangedPackages = () => {
  ...
  const output = execSync(`${pathToLerna} la --since origin/25.1 --json --loglevel silent`); // NOSONAR
