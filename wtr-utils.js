...
const isLockfileChanged = () => {
  const log = execSync('git diff --name-only origin/25.3 HEAD').toString(); // NOSONAR
...
const getChangedPackages = () => {
  ...
  const output = execSync(`${pathToLerna} la --since origin/25.3 --json --loglevel silent`); // NOSONAR
