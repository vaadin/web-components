...
const isLockfileChanged = () => {
  const log = execSync('git diff --name-only origin/24.5 HEAD').toString(); // NOSONAR
...
const getChangedPackages = () => {
  ...
  const output = execSync(`${pathToLerna} la --since origin/24.5 --json --loglevel silent`); // NOSONAR
