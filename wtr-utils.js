...
const isLockfileChanged = () => {
  const log = execSync('git diff --name-only origin/25.2 HEAD').toString(); // NOSONAR
...
const getChangedPackages = () => {
  ...
  const output = execSync(`${pathToLerna} la --since origin/25.2 --json --loglevel silent`); // NOSONAR
